...
const SUPPLIERS: Record<string, {
  ...
  nazih:        { label: "Nazih Group",            accent: "#0ea5e9", href: "/catalog/nazih",        logo: "/logos/nazih.png",         initials: "NZ", jsonFile: "/nazih_all_products.json",    normalizeId: (p, i) => p.id || p.url || `naz-${i}` },
  wella:        { label: "Wella Professionals",    accent: "#1a1a1a", href: "/catalog/wella",        logo: "/logos/wella.svg",         initials: "WP", jsonFile: "/wella_products.json",        normalizeId: (p, i) => p.id || p.slug || p.url || `wel-${i}` },
  ...
  skeyndor:     { label: "Skeyndor",               accent: "#7c3aed", href: "/catalog/skeyndor",     logo: "/logos/skeyndor.png",      initials: "SK", jsonFile: "/skeyndor_products.json",     normalizeId: (p, i) => p.id || p.ean || p.url || `sky-${i}` },
                      ...
                      {items.map((item) => (
                        <div key={item.id}
                          ...
                          <div className="grid h-14 w-14 shrink-0 place-items-center overflow-hidden rounded-[10px] border border-slate-200 bg-gradient-to-br from-white to-slate-100">
                            {item.product.photo ? (
                              <img src={item.product.photo} alt="" className="w-full h-full object-cover"
                                onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                            ) : (
