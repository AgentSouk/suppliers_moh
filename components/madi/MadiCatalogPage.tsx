...
interface Product {
  ...
  price: number | null; photo: string | null; photo_sm?: string | null; images?: string[];
...
function generateOrderNumber() {
  ...
  return `MAD-${d.getFullYear()}${pad(d.getMonth()+1)}${pad(d.getDate())}-${pad(d.getHours())}${pad(d.getMinutes())}`;
}


export default function MadiCatalogPage() {
  ...
  const { cart, setCart, location, setLocation, orderHistory, cartTotals,
    removeFromCart, updateQuantity, clearCart, saveToHistory, clearHistory } = useCart("madi");
  ...
  useEffect(() => {
    ...
    setLocation(loc);
    fetch("/madi_products.json")
      .then(r => { if (!r.ok) throw new Error(); return r.json(); })
      .then((data: Product[]) => {
        setProducts(data.map((p, i) => ({ ...p, id: p.sku || p.id || `mad-${i}` })).filter(p => p.photo || p.photo_sm));
        setLoading(false);
  ...
  const addToCart = (product: Product) => {
    ...
    addToGlobalCart({
      ...
      supplier: "madi",
      supplierLabel: "Madi International",
      product: {
        name: product.name || "",
  ...
  const generatePDF = async () => {
    const orderNum = await generatePO({
      cart,
      supplierName: "Madi International",
      supplierPrefix: "MDI",
      location,
    });
  ...
  const generateExcel = () => {
    const rows = cart.map(i => ({
      "Item Code": (i.product as Product).sku || "",
      Brand: i.product.brand || "",
      Product: i.product.name,
      Qty: i.quantity,
      Price: i.product.price || "",
      Total: (i.product.price || 0) > 0 ? ((i.product.price || 0) * i.quantity).toFixed(2) : "",
      "Photo URL": (i.product as Product).photo || "",
      URL: i.product.url || "",
    }));
    const ws = XLSX.utils.json_to_sheet(rows);
    ws["!cols"] = [15, 25, 55, 8, 10, 10, 70, 70].map(w => ({ wch: w }));
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Madi Order");
  ...
  return (
    <div className="min-h-screen" style={{ background: colors.contentBg }}>
      ...
      {/* Breadcrumb */}
      <div className="px-6 pt-3 pb-1 text-xs bg-white/70 border-b border-gray-100" style={{ color: colors.textMuted }}>
        <a href="/suppliers" className="hover:underline text-blue-500">Suppliers</a>
        <span className="mx-1">›</span><span>Madi International</span>
      </div>
      ...
      <div className="px-6 py-4 bg-white/80 backdrop-blur border-b border-gray-100 shadow-sm sticky top-0 z-40">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            <img src="/logos/madi.svg" alt="Madi" className="h-6 sm:h-8 object-contain" onError={e => (e.target as HTMLImageElement).style.display='none'} />
            <div>
              <h1 className="text-xs sm:text-lg font-bold text-gray-900">Madi International Catalogue</h1>
              <p className="text-xs text-gray-400">{products.length} products · {brands.length} brands</p>
            </div>
          </div>
        ...
        <div className="relative max-w-sm">
          ...
          <input className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-lg text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-gray-400"
            placeholder="Search by name, item code, brand…" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
        </div>
      </div>

      {/* Category / Sub-category / Brand nav */}
      <div className="bg-white/80 border-b border-gray-100 px-6 py-2">
        ...
        {selectedCategory && subCategories.length > 1 && (
          <div className="flex gap-1.5 overflow-x-auto pt-1.5">
            <button onClick={() => setSelectedSubCategory(null)}
              className={`px-2.5 py-1 rounded-full text-[11px] whitespace-nowrap transition-all ${!selectedSubCategory ? "bg-gray-200 text-gray-700 font-semibold" : "text-gray-400 hover:text-gray-600"}`}>
              All {selectedCategory}
            </button>
            {subCategories.map(sub => (
              <button key={sub} onClick={() => setSelectedSubCategory(sub)}
                className={`px-2.5 py-1 rounded-full text-[11px] whitespace-nowrap transition-all ${selectedSubCategory === sub ? "bg-gray-200 text-gray-700 font-semibold" : "text-gray-400 hover:text-gray-600"}`}>
                {sub}
              </button>
            ))}
          </div>
        ...
        {/* Brand filter */}
        {brands.length > 0 && (
          <div className="flex gap-1.5 overflow-x-auto pt-1.5 border-t border-gray-100 mt-1.5">
          ...
          <div className="relative w-full max-w-lg h-full overflow-y-auto bg-white flex flex-col">
            <div className="sticky top-0 px-6 py-4 flex items-center justify-between bg-white border-b border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg font-bold text-gray-900">Cart · Madi</h2>
                <span className="px-2 py-0.5 rounded-full text-xs font-bold text-white bg-gray-900">{cart.length}</span>
              </div>
              <div className="flex items-center gap-2">
            ...
            {showHistory && (
              <div className="p-6">
                <div className="flex items-center justify-between mb-3">
                  ...
                  {orderHistory.length > 0 && <button onClick={() => { if (confirm("Clear?")) clearHistory(); }} className="text-xs text-red-400 hover:text-red-600">Clear</button>}
                </div>
                  ...
                  cart.map(item => (
                    <div key={item.id} className="flex gap-4 p-4 rounded-xl border border-gray-100 bg-gray-50">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {(item.product as Product).sku && <p className="text-xs text-gray-400 font-mono">{(item.product as Product).sku}</p>}
                        <div className="flex items-center gap-2 mt-2">
                          ...
                          <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="w-7 h-7 rounded flex items-center justify-center text-white bg-gray-900"><Plus className="w-3 h-3" /></button>
                        </div>
                      </div>
                      <div className="flex flex-col items-end justify-between">
                        <button onClick={() => removeFromCart(item.id)}><Trash2 className="w-4 h-4 text-red-400" /></button>
                        {(item.product.price || 0) > 0 && <p className="text-sm font-bold text-gray-900">{((item.product.price || 0) * item.quantity).toFixed(2)}</p>}
                      </div>
                    </div>
                  ))
                }
              ...
              <div className="sticky bottom-0 p-6 border-t bg-white">
                ...
                {cartTotals.totalValue > 0 && (
                  <div className="flex justify-between text-lg font-bold pt-2 border-t border-gray-100 mb-4">
                    <span>Total</span><span className="text-gray-900">{cartTotals.totalValue.toFixed(2)}</span>
                  </div>
                )}
                <div className="grid grid-cols-2 gap-2 mb-3">
                  <button onClick={generatePDF} className="flex items-center justify-center gap-1.5 border border-gray-300 text-gray-700 rounded-lg py-2.5 text-sm hover:bg-gray-50">
                ...
                <ShareCartButton cart={cart} location={location} supplierId="madi" supplierLabel="Madi International" />
