...
const colors = {
  ...
  cardBg: "#ffffff", contentBg: "linear-gradient(135deg, #fffbeb 0%, #f8faff 100%)",
...
function generateOrderNumber() {
  ...
  return `NW-${d.getFullYear()}${pad(d.getMonth()+1)}${pad(d.getDate())}-${pad(d.getHours())}${pad(d.getMinutes())}`;
}


export default function NawajmCatalogPage() {
  ...
  const { cart, setCart, location, setLocation, orderHistory, cartTotals,
    removeFromCart, updateQuantity, clearCart, saveToHistory, clearHistory } = useCart("nawajm");
  ...
  const generatePDF = async () => {
    ...
    doc.setFontSize(20); doc.setTextColor(217, 119, 6);
    doc.text("NAWAJM COSMETICS — PURCHASE ORDER", pageWidth / 2, currentY, { align: "center" });
    currentY += 10;
  ...
  const generateExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Nawajm Order");
    ...
    <div className="min-h-screen" style={{ background: colors.contentBg }}>
      ...
      <div className="px-6 py-4 bg-white/80 backdrop-blur border-b border-amber-50 shadow-sm sticky top-0 z-40">
        ...
        <div className="relative max-w-sm">
          ...
          <input className="w-full pl-9 pr-4 py-2 border border-amber-100 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-amber-400"
            placeholder="Search by name, SKU, brand…" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
        </div>
      </div>

      {/* Category / Brand nav */}
      <div className="bg-white/80 border-b border-amber-50 px-6 py-2">
        <div className="flex gap-1.5 overflow-x-auto pb-1.5">
          <button onClick={() => { setSelectedCategory(null); setSelectedBrand(null); }}
            className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-all ${!selectedCategory ? "text-white" : "bg-white text-gray-500 border border-amber-100 hover:bg-amber-50"}`}
            style={!selectedCategory ? { background: colors.primary } : {}}>
            All
          </button>
          {categories.map(cat => (
            <button key={cat} onClick={() => { setSelectedCategory(cat); setSelectedBrand(null); }}
              className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-all ${selectedCategory === cat ? "text-white" : "bg-white text-gray-500 border border-amber-100 hover:bg-amber-50"}`}
        ...
        {brands.length > 0 && (
          <div className="flex gap-1.5 overflow-x-auto pt-1.5 border-t border-amber-50 mt-1.5">
      ...
      <div className="p-6">
        ...
        {filteredProducts.length === 0 ? (
          <div className="text-center py-24 text-gray-400">
            ...
            <p className="text-sm">Run <code className="bg-gray-100 px-2 py-0.5 rounded">python3 nawajm_scraper.py</code> then copy to <code className="bg-gray-100 px-2 py-0.5 rounded">public/</code></p>
          </div>
        ) : (
                    ...
                    {product.photo ? (
                      ...
                      <div className="w-full h-full flex flex-col items-center justify-center gap-2">
                        <div className="w-12 h-12 rounded-full flex items-center justify-center" style={{ background: "#fef3c7" }}>
                          <span className="text-lg font-bold" style={{ color: colors.primary }}>NW</span>
                        </div>
