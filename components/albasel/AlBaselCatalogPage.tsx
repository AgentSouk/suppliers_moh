...
const colors = {
  primary: "#b8860b", primaryHover: "#8b6508",
  cardBg: "#ffffff", contentBg: "linear-gradient(135deg, #fefce8 0%, #f8faff 100%)",
...
function generateOrderNumber() {
  ...
  return `AB-${d.getFullYear()}${pad(d.getMonth()+1)}${pad(d.getDate())}-${pad(d.getHours())}${pad(d.getMinutes())}`;
}


export default function AlBaselCatalogPage() {
  ...
  const { cart, setCart, location, setLocation, orderHistory, cartTotals,
    removeFromCart, updateQuantity, clearCart, saveToHistory, clearHistory } = useCart("albasel");
  ...
  useEffect(() => {
    ...
    setLocation(loc);
    fetch("/albasel_products.json")
      .then(r => { if (!r.ok) throw new Error(); return r.json(); })
      .then((data: Product[]) => {
        setProducts(data.map((p, i) => ({ ...p, id: p.id || p.sku || `ab-${i}` })).filter(p => p.photo));
        setLoading(false);
  ...
  const addToCart = (product: Product) => {
    ...
    addToGlobalCart({
      ...
      supplier: "albasel",
      supplierLabel: "Al Basel Cosmetics",
      product: {
        name: product.name || "",
  ...
  const generatePDF = async () => {
    ...
    doc.text("AL BASEL COSMETICS — PURCHASE ORDER", pageWidth / 2, currentY, { align: "center" });
    currentY += 10;
    doc.setFontSize(9); doc.setTextColor(100, 100, 100);
    doc.text(`Order #: ${orderNumber}  ·  Date: ${new Date().toLocaleDateString()}  ·  Salon: ${location}`, pageWidth / 2, currentY, { align: "center" });
    currentY += 15;

    const itemsPerPage = 4;
    const chunks: typeof cart[] = [];
    for (let i = 0; i < cart.length; i += itemsPerPage) chunks.push(cart.slice(i, i + itemsPerPage));

    for (let pageIdx = 0; pageIdx < chunks.length; pageIdx++) {
      if (pageIdx > 0) { doc.addPage(); currentY = 20; }
      for (const item of chunks[pageIdx]) {
        ...
        if (p.photo) {
          try {
            const resp = await fetch(`/api/image-proxy?url=${encodeURIComponent(p.photo)}`);
            if (resp.ok) {
              ...
              doc.addImage(b64, "JPEG", margin + 2, boxY + 2, imgSize, imgSize - 4);
            }
          } catch {}
        }
        ...
        if (p.category) doc.text(p.category, textX, boxY + 6);
        doc.setFontSize(7); doc.setTextColor(100, 100, 100);
        if (p.brand) doc.text(p.brand, textX, boxY + 11);
        doc.setFontSize(9); doc.setTextColor(17, 24, 39);
        const nameLines = doc.splitTextToSize(p.name, pageWidth - textX - margin - 30);
        doc.text(nameLines.slice(0, 2), textX, boxY + 18);
        doc.setFontSize(7.5); doc.setTextColor(100, 100, 100);
        if (p.sku) doc.text(`SKU: ${p.sku}`, textX, boxY + 28);

        const qtyColX = pageWidth - margin - 28;
        doc.setFontSize(8); doc.setTextColor(17, 24, 39);
        doc.text(`Qty: ${item.quantity}`, qtyColX, boxY + 12);
        if (p.price) {
          doc.text(`${p.price.toFixed(2)}`, qtyColX, boxY + 20);
          doc.setTextColor(120, 120, 120);
          doc.text(`Line: ${(p.price * item.quantity).toFixed(2)}`, pageWidth - margin - 2, boxY + 36, { align: "right" });
        }
        currentY += boxHeight + 4;
      }
      doc.setFontSize(8); doc.setTextColor(150, 150, 150);
      doc.text(`Page ${pageIdx + 1} of ${chunks.length}`, pageWidth / 2, pageHeight - 14, { align: "center" });
    }
    ...
    autoTable(doc, {
      startY: currentY,
      head: [["SKU", "Brand", "Category", "Product", "Qty", "Price (AED)", "Total (AED)"]],
      body: cart.map(i => [
        (i.product as Product).sku || "—",
        (i.product as Product).brand || "—",
        (i.product as Product).category || "—",
        i.product.name,
        i.quantity,
    ...
    doc.setFontSize(12); doc.setTextColor(184, 134, 11);
    doc.text(`Total Items: ${cartTotals.totalQty}`, margin, finalY);
    if (cartTotals.totalValue > 0)
      doc.text(`TOTAL: ${cartTotals.totalValue.toFixed(2)} AED`, pageWidth - margin, finalY, { align: "right" });
    ...
    saveToHistory(orderNumber, cartTotals.totalValue);
  };
  ...
  const generateExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Al Basel Order");
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: colors.contentBg }}>
      <div className="w-8 h-8 rounded-full border-4 border-yellow-200 border-t-yellow-600 animate-spin" />
    </div>
  );
      ...
      <div className="px-6 py-4 bg-white/80 backdrop-blur border-b border-yellow-50 shadow-sm sticky top-0 z-40">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            <img src="/logos/albasel.svg" alt="Al Basel Cosmetics" className="h-8 object-contain" onError={e => (e.target as HTMLImageElement).style.display='none'} />
            <div>
              <h1 className="text-lg font-bold text-gray-900">Al Basel Cosmetics Catalogue</h1>
        ...
        <div className="relative max-w-sm">
          ...
          <input className="w-full pl-9 pr-4 py-2 border border-yellow-100 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-yellow-400"
            placeholder="Search by name, SKU, brand…" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
        </div>
      </div>

      {/* Category / Brand nav */}
      <div className="bg-white/80 border-b border-yellow-50 px-6 py-2">
        <div className="flex gap-1.5 overflow-x-auto pb-1.5">
          <button onClick={() => { setSelectedCategory(null); setSelectedBrand(null); }}
            className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-all ${!selectedCategory ? "text-white" : "bg-white text-gray-500 border border-yellow-100 hover:bg-yellow-50"}`}
            style={!selectedCategory ? { background: colors.primary } : {}}>
            All
          </button>
          {categories.map(cat => (
            <button key={cat} onClick={() => { setSelectedCategory(cat); setSelectedBrand(null); }}
              className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-all ${selectedCategory === cat ? "text-white" : "bg-white text-gray-500 border border-yellow-100 hover:bg-yellow-50"}`}
        ...
        {brands.length > 0 && (
          <div className="flex gap-1.5 overflow-x-auto pt-1.5 border-t border-yellow-50 mt-1.5">
              ...
              return (
                <div key={product.id} className="rounded-xl border overflow-hidden transition-shadow hover:shadow-md flex flex-col"
                  ...
                  <div className="relative bg-gray-50 overflow-hidden" style={{ height: "12rem" }}>
                    {product.photo ? (
                      ...
                      <div className="w-full h-full flex flex-col items-center justify-center gap-2">
                        <div className="w-12 h-12 rounded-full flex items-center justify-center" style={{ background: "#fef9c3" }}>
                          <span className="text-lg font-bold" style={{ color: colors.primary }}>AB</span>
                        </div>
                        <span className="text-xs text-gray-300">No image</span>
                      </div>
                    )}
                    {product.available === false && (
                      <div className="absolute top-2 left-2 px-2 py-0.5 rounded-full text-[10px] font-medium bg-red-100 text-red-600">Out of stock</div>
                    )}
                    {cartItem && (
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    <div className="flex-1 mb-3 cursor-pointer group" onClick={() => setDetailProduct(product)}>
                      <div className="flex items-center justify-between mb-0.5">
                        <p className="text-xs font-semibold uppercase tracking-wide" style={{ color: colors.primary }}>{product.brand}</p>
                        {product.category && <p className="text-[10px] text-gray-400">{product.category}</p>}
                      </div>
                      <h3 className="text-sm font-semibold leading-tight line-clamp-2 mb-1 group-hover:underline" style={{ color: colors.text }}>{product.name}</h3>
                      {product.sku && <p className="text-xs text-gray-400">SKU: <span className="font-mono text-gray-600">{product.sku}</span></p>}
                      {product.description && <p className="text-xs text-gray-400 line-clamp-1 mt-0.5">{product.description}</p>}
                    </div>
                    <div className="flex items-center justify-between mt-auto">
                      <span className="text-lg font-bold" style={{ color: colors.primary }}>
                        {product.price ? `${product.price.toFixed(2)}` : "—"}
                        {product.price && <span className="text-xs font-normal text-gray-400 ml-1">AED</span>}
                      </span>
                      {cartItem ? (
                        <div className="flex items-center gap-2">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
            ...
            {detailProduct.photo && (
              <div className="bg-gray-50 px-6 py-4 flex justify-center">
                <img src={detailProduct.photo} alt={detailProduct.name} className="max-h-52 object-contain rounded-lg" />
              </div>
            )}
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-lg h-full overflow-y-auto bg-white flex flex-col">
            <div className="sticky top-0 px-6 py-4 flex items-center justify-between bg-white border-b border-yellow-50">
              <div className="flex items-center gap-3">
                <ShoppingCart className="w-5 h-5" style={{ color: colors.primary }} />
                <h2 className="text-lg font-bold text-gray-900">Cart · Al Basel</h2>
            ...
            {!showHistory && cart.length > 0 && (
              <div className="sticky bottom-0 p-6 border-t bg-white">
                ...
                <ShareCartButton cart={cart} location={location} supplierId="albasel" supplierLabel="Al Basel Cosmetics" />
