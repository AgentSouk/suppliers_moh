...
function generateOrderNumber() {
  ...
  return `SKY-${d.getFullYear()}${pad(d.getMonth()+1)}${pad(d.getDate())}-${pad(d.getHours())}${pad(d.getMinutes())}`;
}


export default function SkeyndorCatalogPage() {
  ...
  const { cart, setCart, location, setLocation, orderHistory, cartTotals,
    removeFromCart, updateQuantity, clearCart, saveToHistory, clearHistory } = useCart("skeyndor");
  ...
  const generateExcel = () => {
    const rows = cart.map(i => ({
      "EAN Barcode": (i.product as Product).ean || "",
      Line: (i.product as Product).category || "",
      Product: i.product.name,
      Qty: i.quantity,
      Price: i.product.price || "",
      Currency: (i.product as Product).currency || "EUR",
      Total: i.product.price ? (i.product.price * i.quantity).toFixed(2) : "",
      Description: (i.product as Product).description || "",
      "Photo URL": (i.product as Product).photo || "",
      URL: i.product.url || "",
    }));
    const ws = XLSX.utils.json_to_sheet(rows);
    ws["!cols"] = [15, 20, 50, 8, 10, 8, 10, 60, 60, 60].map(w => ({ wch: w }));
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Skeyndor Order");
  ...
  return (
    <div className="min-h-screen" style={{ background: colors.contentBg }}>
      ...
      {/* Breadcrumb */}
      <div className="px-6 pt-3 pb-1 text-xs bg-white/70 border-b border-purple-50" style={{ color: colors.textMuted }}>
        <a href="/suppliers" className="hover:underline" style={{ color: colors.primary }}>Suppliers</a>
        <span className="mx-1">›</span><span>Skeyndor</span>
      </div>

      {/* Header */}
      <div className="px-6 py-4 bg-white/80 backdrop-blur border-b border-purple-50 shadow-sm sticky top-0 z-40">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            <img src="/logos/skeyndor.png" alt="Skeyndor" className="h-8 object-contain" onError={e => (e.target as HTMLImageElement).style.display='none'} />
            <div>
              <h1 className="text-lg font-bold text-gray-900">Skeyndor Catalogue</h1>
        ...
        <div className="relative max-w-sm">
          ...
          <input className="w-full pl-9 pr-4 py-2 border border-purple-100 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-purple-400"
            placeholder="Search by name, EAN, description…" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
        </div>
      </div>

      {/* Category tabs */}
      <div className="bg-white/80 border-b border-purple-50 px-6 py-2">
        <div className="flex gap-1.5 overflow-x-auto">
          <button onClick={() => setSelectedCategory(null)}
            className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-all ${!selectedCategory ? "text-white" : "bg-white text-gray-500 border border-purple-100 hover:bg-purple-50"}`}
          ...
          {categories.map(cat => {
            ...
            return (
              <button key={cat} onClick={() => setSelectedCategory(cat)}
                className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-all ${selectedCategory === cat ? "text-white" : "bg-white text-gray-500 border border-purple-100 hover:bg-purple-50"}`}
      ...
      <div className="p-6">
        <p className="text-xs text-gray-400 mb-4">
          {filteredProducts.length} products{selectedCategory ? ` · ${selectedCategory}` : ""}
        </p>

        {filteredProducts.length === 0 ? (
          <div className="text-center py-24 text-gray-400">
            <p className="text-lg mb-2">No products yet</p>
            <p className="text-sm">Run <code className="bg-gray-100 px-2 py-0.5 rounded">python3 skeyndor_scraper.py</code> then copy to <code className="bg-gray-100 px-2 py-0.5 rounded">public/</code></p>
          </div>
        ) : (
                ...
                <div key={product.id} className="rounded-xl border overflow-hidden transition-shadow hover:shadow-md flex flex-col"
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    <div className="flex-1 mb-3">
                      ...
                      {product.description && <p className="text-xs text-gray-400 line-clamp-2 leading-relaxed">{product.description}</p>}
                    </div>
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-lg h-full overflow-y-auto bg-white flex flex-col">
            <div className="sticky top-0 px-6 py-4 flex items-center justify-between bg-white border-b border-purple-50">
              <div className="flex items-center gap-3">
                <ShoppingCart className="w-5 h-5" style={{ color: colors.primary }} />
                <h2 className="text-lg font-bold text-gray-900">Cart · Skeyndor</h2>
                  ...
                  cart.map(item => (
                    <div key={item.id} className="flex gap-4 p-4 rounded-xl border border-gray-100 bg-gray-50">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {(item.product as Product).ean && <p className="text-xs text-gray-400">EAN: {(item.product as Product).ean}</p>}
              ...
              <div className="sticky bottom-0 p-6 border-t bg-white">
                ...
                {cartTotals.totalValue > 0 && (
                  <div className="flex justify-between text-lg font-bold pt-2 border-t border-gray-100 mb-4">
                    <span>Total</span><span style={{ color: colors.primary }}>{cartTotals.totalValue.toFixed(2)}</span>
                  </div>
                )}
                ...
                <ShareCartButton cart={cart} location={location} supplierId="skeyndor" supplierLabel="Skeyndor" />
