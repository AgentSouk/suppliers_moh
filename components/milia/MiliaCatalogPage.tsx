...
export default function MiliaCatalogPage() {
  ...
  useEffect(() => {
    ...
    fetch("/milia_products.json")
      ...
      .then((data: Product[]) => {
        setProducts(data.map((p, i) => ({ ...p, id: p.id || p.sku || `ml-${i}` })));
        setLoading(false);
      ...
      <div className="px-6 py-4 bg-white/80 backdrop-blur border-b border-teal-50 shadow-sm sticky top-0 z-40">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            <img src="/logos/milia.png" alt="Milia Cosmetics" className="h-8 object-contain" onError={e => (e.target as HTMLImageElement).style.display='none'} />
            <div>
              <h1 className="text-lg font-bold text-gray-900">Milia Cosmetics Catalogue</h1>
                  ...
                  <div className="relative bg-gray-50 overflow-hidden" style={{ height: "12rem" }}>
                    {product.photo ? (
                      ...
                      <div className="w-full h-full flex flex-col items-center justify-center gap-2">
                        <div className="w-12 h-12 rounded-full flex items-center justify-center" style={{ background: "#ccfbf1" }}>
                          <span className="text-lg font-bold" style={{ color: colors.primary }}>ML</span>
                        </div>
